...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json(other); //only sending other here
  } catch (err) {
...
//get friends {only getting image, username and userId}
router.get("/friends/:userId", async(req, res) => {
  try {
    ...
    ) //Promise because again, I'll be using map()
  ...
  if (req.body.userId !== req.params.id) {
    try {
      ...
      if (!user.followers.includes(req.body.userId)) {
        //if who the current user is trying to follow isn't already included in the user's followers
    ...
    res
      .status(400)
      .json("You cannot follow yourself. Please select another user");
  }
});
...
router.put("/:id/unfollow", async (req, res) => {
  if (req.body.userId !== req.params.id) {
    ...
    res
      .status(400)
      .json("You cannot un-follow yourself. Please select another user");
  }
});
